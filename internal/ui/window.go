...
var (
	defaultWindowSize       = fyne.NewSize(1440, 900)
	minimumDashboardSize    = fyne.NewSize(1280, 800)
	hostsPanelMinSize       = fyne.NewSize(540, 0)
	resultInspectorSize     = fyne.NewSize(1200, 780)
	resultInspectorMinSize  = fyne.NewSize(960, 640)
	toolbarMinSize          = fyne.NewSize(0, 128)
	toolbarWarningHeight    = float32(24)
	operationContentMin     = fyne.NewSize(840, 430)
	operationCardMaxWidth   = float32(880)
	operationFieldWidth     = float32(780)
	operationBrowseWidth    = float32(120)
	operationBrowseInput    = float32(640)
	operationButtonMinSize  = fyne.NewSize(200, 46)
	viewResultsButtonSize   = fyne.NewSize(170, 40)
	statusBarMinSize        = fyne.NewSize(0, 64)
	knownHostsControlSize   = fyne.NewSize(520, 38)
	toolbarSelectSize       = fyne.NewSize(120, 38)
	languageSelectSize      = fyne.NewSize(220, 38)
	insecureCheckSize       = fyne.NewSize(240, 38)
	hostActionButtonSize    = fyne.NewSize(220, 38)
	hostWideButtonSize      = fyne.NewSize(460, 38)
	hostsEmptyStateSize     = fyne.NewSize(500, 86)
	hostTableColumnDefaults = []float32{60, 190, 80, 150, 150}
	defaultSplitOffset      = float64(0.40)
)
